...
if os.environ.get('GAE_ENV') == 'standard':
    ...
    cloud_sql_connection_name = os.environ.get('CLOUD_SQL_CONNECTION_NAME', 'total-handler-244211:us-central1:saas1')
...
def static_proxy(path):
    # APIエンドポイントは除外
    if path.startswith(('save', 'customers', 'health', 'api')):
...
def save_data():
    ...
    try:
        # Support both old field names (for backward compatibility) and new field names
        customer = Customer(
            name=data.get("name", data.get("newCustomerName")),
            plan=data.get("plan", data.get("newCustomerPlan")),
            mrr=int(data.get("mrr", data.get("newCustomerMrr", 0))),
            initial_fee=int(data.get("initialFee", data.get("newCustomerInitialFee", 0))),
            operation_fee=int(data.get("operationFee", data.get("newCustomerOperationFee", 0))),
            assignee=data.get("assignee", data.get("newCustomerAssignee")),
            hours=int(data.get("hours", data.get("newCustomerHours", 0))),
            region=data.get("region", data.get("newCustomerRegion")),
            industry=data.get("industry", data.get("newCustomerIndustry")),
            channel=data.get("channel", data.get("newCustomerChannel")),
            status=data.get("status", data.get("newCustomerStatus", "active")),
            contract_date=data.get("startDate", data.get("newCustomerContractDate")),
            health_score=int(data.get("healthScore", 70)),
            last_login=data.get("lastLogin"),
            support_tickets=int(data.get("supportTickets", 0)),
            nps_score=int(data.get("npsScore", 7)),
            usage_rate=int(data.get("usageRate", 50)),
